...
- [ ] 3.2 En el POS: apretar `1`–`9` y ver que la sección no cambia; apretar `Enter` con productos en el ticket y ver que no cobra
- [ ] 3.3 En el POS: cambiar de sección con clic y cobrar con el botón del ticket, y confirmar que el flujo sigue igual
- [ ] 3.4 En el POS: elegir una sección con el clic y apretar cualquier tecla, y ver que el borde verde no cambia ni aparece un anillo blanco
- [ ] 3.5 En el POS: mirar las tarjetas de sección (sin numeritos) y las de almuerzo (número de unidades solo) para confirmar que la confusión desapareció
- [ ] 3.6 En el POS: escribir el nombre del cliente y presionar `Enter` en ese campo, y cerrar con `Escape` un menú emergente, para confirmar que eso sigue igual
